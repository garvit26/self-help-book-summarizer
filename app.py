...
from langchain_chroma import Chroma  # ← NEW FIX FOR CHROMA
...
st.title("📚 Self-Help Book Chapter-Wise Summarizer")
...
def extract_chapters(pdf_file, book_name):
    ...
    if toc:
        # Same as before — your original TOC logic
        chapters = []
...
if uploaded_files:
    for uploaded_file in uploaded_files:
        ...
        with st.spinner(f"Processing {uploaded_file.name}..."):
            ...
            st.success(f"✅ Extracted {len(new_chapters)} chapters from {uploaded_file.name}")
...
if st.session_state.chapters:
    ...
    for i, chap in enumerate(st.session_state.chapters):
        with st.expander(f"**{chap.metadata['book_title']}** — {chap.metadata['chapter_title']} (Pages {chap.metadata['start_page']}-{chap.metadata['end_page']})"):
...
st.markdown("---")
st.caption("Built with ❤️ using Streamlit + LangChain + Ollama (100% local & private)")